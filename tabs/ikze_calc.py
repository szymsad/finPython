...
def render(params):
    ...
    miesiac_start = st.number_input(
        ...
        min_value=1, max_value=12, value=datetime.date.today().month, step=1, key="ikze_month_start"
    )
            ...
            try:
                ...
                if hist.empty or len(hist) < params['long_span'] + 5:
                    macd_sygnaly.append({"Spółka": t, "Nazwa": info["nazwa"], "Tier": info["tier"],
                                         "Kurs (zł)": "—", "MACD": "—", "Signal": "—", "RSI": "—", "EMA200": "—",
    ...
    with st.spinner(f"Pobieram dane i symuluję strategię dla {ikze_ticker}..."):
        ...
        if not hist_w.empty:
            ...
            hist_w['MACD'] = hist_w['Close'].ewm(span=params['short_span']).mean() - hist_w['Close'].ewm(
            ...
            for i in range(len(macd_w) - 1):
                ...
                if (macd_w.iloc[i + 1] > sig_w.iloc[i + 1] and macd_w.iloc[i] < sig_w.iloc[i]) or \
                    ...
                    if (i - ikze_ostatnia_transakcja_idx) >= params['cooldown_param']:
                        ...
                        if macd_w.iloc[i + 1] > sig_w.iloc[i + 1]:
                            if rsi_i < params['rsi_kupno'] and (
                                    not params['uzywaj_ema200'] or ponad_ema_i) and dobry_vol_i:
                                pct = params['duzy_wykup_pct'] if y <= params['poziom_dolka'] else params[
                                    'maly_wykup_pct']
                                ...
                                if ikze_saldo != stre:
                                    ...
                                    ikze_historia.append(
                                        {'typ': 'KUP', 'data': data_i, 'punkt_data': punkt_data, 'cena': cena_i,
                                         'y_macd': y, 'kolor': kol, 'rozmiar': s})
                        else:
                            if rsi_i > params['rsi_sprzedaz'] and dobry_vol_i:
                                pct = params['duza_gorka_pct'] if y >= params['poziom_gorki'] else params[
                                    'mala_gorka_pct']
                                ...
                                if ikze_amount != stry:
                                    ikze_ostatnia_transakcja_idx = i
                                    ikze_historia.append(
                                        {'typ': 'SPRZEDAJ', 'data': data_i, 'punkt_data': punkt_data, 'cena': cena_i,
                                         'y_macd': y, 'kolor': kol, 'rozmiar': s})
            ...
            ax2.axhline(params['poziom_gorki'], color='red', linewidth=0.8, linestyle=':', alpha=0.6,
                        label='Próg górki')
            ax2.axhline(params['poziom_dolka'], color='green', linewidth=0.8, linestyle=':', alpha=0.6,
                        label='Próg dołka')
            ...
            ax3.axhline(params['rsi_kupno'], color='red', linewidth=0.8, linestyle=':', alpha=0.7,
                        label=f'RSI kupno ({params["rsi_kupno"]})')
            ax3.axhline(params['rsi_sprzedaz'], color='green', linewidth=0.8, linestyle=':', alpha=0.7,
                        label=f'RSI sprzedaż ({params["rsi_sprzedaz"]})')
            ax3.axhline(50, color='gray', linewidth=0.6, linestyle='--', alpha=0.4)
            ax3.fill_between(daty_w, rsi_w, params['rsi_kupno'], where=(rsi_w > params['rsi_kupno']), alpha=0.15,
                             color='red')
            ax3.fill_between(daty_w, rsi_w, params['rsi_sprzedaz'], where=(rsi_w < params['rsi_sprzedaz']), alpha=0.15,
                             color='green')
            ax3.set_ylim(0, 100);
            ...
            if ikze_historia:
                st.caption(
                    "▲ zielony = kupno | ▼ czerwony/pomarańczowy = sprzedaż | duży = mocny sygnał | mały = zwykły sygnał")